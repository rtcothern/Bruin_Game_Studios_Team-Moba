...
void UAbilityComponent::CastProjectile()
{
	...
	FRotator Rot = FRotationMatrix::MakeFromX(FVector(TargetLocation, 0) - GetCaster()->GetActorLocation()).Rotator();
	...
	else
	{
		...
		Projectile->SetDestination(TargetLocation, MaxRange);// FVector(TargetLocation.X, TargetLocation.Y, 200));
	}
}
